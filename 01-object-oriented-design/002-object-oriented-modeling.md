...
***Questions***<br>
1. Choose the attributes that will be **encapsulated** in each of the above objects<br>
...
***Questions***<br>
1. Whare are some objects that you might get by decomposing a car?
...
2. Which of these car "parts" has a dynamic number?<br>
...
1. Inheritance is used to describe the relationship between classes. What are the three ways in which this relationship is advantageous?
    * Subclasses that inherit from the same superclass include attributes and behaviours from the superclass<br>
        *(This is an easy way to share attributes and behaviour among related classes)*
    * Changes can be made easily and quickly to a large group of subclasses by making changes to the superclass they inherit from<br>
        *(Changes to the superclass apply to all the subclasses unless the subclasses overrides the superclass)*
    * Code can be reused through inheritance, which means we do not need to re-implement behaviours that already exist<br>
